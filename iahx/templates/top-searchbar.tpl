        ...
        <div class="search">
            <form name="searchForm" action="{$smarty.server.PHP_SELF}" method="post">
                ...
                <div class="searchForm">
                        ...
                        <input type="text" name="q" value="{if $q_escaped eq ''}{$texts.ENTER_WORDS}{else}{$q_escaped}{/if}" class="inputText defaultValue" onKeyDown="if(event.keyCode==13) newSearch();" onblur="clearDefault('textEntry1', 'inputText defaultValue'); this.value= (this.value=='')? '{$texts.ENTER_WORDS}' : this.value" onfocus="clearDefault('textEntry1', 'inputText'); this.value= (this.value=='{$texts.ENTER_WORDS}')? '' : this.value"  id="textEntry1"/>
